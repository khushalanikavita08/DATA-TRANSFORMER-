...
🔍 Queries Covered (Joins, Subqueries, Date, String, Window, CASE)
...
People who register on the platform
...
Staff working in different departments with different salaries
...
CREATE DATABASE, CREATE TABLE with data types

🔑 Keys & constraints

PRIMARY KEY on every table and FOREIGN KEY between orders and customers

📥 Data insertion

INSERT INTO ... VALUES with multiple rows in one statement
...
Filtering rows using AVG() calculated in an inner query

📅 Date functions

EXTRACT, date subtraction with CURRENT_DATE, TO_CHAR
...
Running total with SUM() OVER, ranking with RANK() OVER

🎛️ Conditional logic

CASE WHEN ... THEN ... ELSE ... END for discounts and salary bands
...
PostgreSQL (version 12 or above recommended)

💾 Language

SQL: DDL (create tables), DML (insert data), DQL (select queries)

🖥️ Tools

psql command line, pgAdmin, DBeaver or any PostgreSQL client
...
FOREIGN KEY → Customers_News(CustomerID)
...
Order value with 2 decimal places
...
Department such as Sales or HR
...
Orders_News.CustomerID must always match an existing Customers_News.CustomerID, so an order can never belong to a customer that does not exist
...
Get all orders together with the details of the customer who placed them

💡 How it works

Returns only the rows where CustomerID exists in both tables. Customers without orders (and orders without customers) are left out
...
List all customers, along with their orders if they have any

💡 How it works

Every row of the left table (Customers_News) is kept. If a customer has no order, the order columns show NULL. Useful to find customers who never ordered
...
List all orders, along with their customer details if available

💡 How it works

Every row of the right table (Orders_News) is kept. It is the mirror image of a LEFT JOIN
...
Combines LEFT and RIGHT joins. Unmatched rows from either side appear with NULL in the other table's columns
...
Find customers who placed an order worth more than the average order

💡 How it works

The inner query SELECT AVG(TotalAmount) returns 175.625. The outer query keeps only orders with TotalAmount > 175.625
...
The average salary is (50000 + 55000) / 2 = 52500, so only employees earning more than that are returned
...
Split an order date into separate year and month columns, useful for monthly or yearly reports

💡 How it works

EXTRACT(part FROM date) pulls out one part of a date such as YEAR, MONTH or DAY
...
Find how many days ago each order was placed

💡 How it works

In PostgreSQL, subtracting one DATE from another gives the number of days. CURRENT_DATE is today's date
...
Show dates in an easy-to-read format

💡 How it works

TO_CHAR(date, 'format') converts a date to text. DD = day, Mon = short month name, YYYY = 4-digit year
...
CONCAT() joins values together. A space ' ' is added in the middle
...
Replace the text John with Jonathan in first names

💡 How it works

REPLACE(text, old, new) swaps every occurrence of old with new. It only changes the query result, not the stored data
...
Show first names in CAPITAL letters and last names in small letters

💡 How it works

UPPER() converts text to capitals and LOWER() converts it to small letters
...
Clean the email column by removing spaces at the start and end

💡 How it works

TRIM() removes leading and trailing spaces. It is very useful when cleaning messy data
...
Show the cumulative sum of order amounts, in order of date

💡 How it works

SUM(...) OVER (ORDER BY OrderDate ...) adds the current order to all earlier orders. UNBOUNDED PRECEDING means "from the first row", up to the CURRENT ROW
...
Give rank 1 to the highest-value order, rank 2 to the next, and so on

💡 How it works

RANK() OVER (ORDER BY TotalAmount DESC) sorts by amount from high to low. If two orders have the same amount they get the same rank and the next rank is skipped
...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:E0A95F,100:C77BA8&section=header&height=110&text=Live%20Sample%20Outputs&fontSize=34&fontColor=ffffff&fontAlignY=40" alt="🚀 Live Sample Outputs">
</p>

🚀 Run the sample outputs interactively

The sample outputs below are based on the same sample data used in this README.

👉 Open Live Sample Outputs

The live page includes:

▶️ Run Query buttons for all 17 queries

▶️ Run All Queries to refresh every output

🔄 A live CURRENT_DATE result for Query 8

🔎 Search/filter for the query cards

📊 Interactive output tables

💡 The HTML page is the interactive companion to this README. The README keeps the SQL explanation and expected outputs, while the HTML page provides the live-style experience.
...
🛠️ Create the database and connect to it:

CREATE DATABASE DataTransformer;
\c DataTransformer

🧱 Create the tables in this order (Customers first, because Orders depends on it):
Customers_News → Orders_News → Employees_News

📥 Insert the sample data into each table.

🔎 Run the 17 queries one by one and compare your output with the tables shown above.
...
Customers without orders and bigger orders, to see every join and CASE result

🗂️ Indexes

Faster searches on CustomerID and OrderDate

👁️ Views

Saved queries for common reports such as customer orders

📈 GROUP BY and aggregates

Total sales per customer or per month
...
NOT NULL, UNIQUE on email, CHECK for positive amounts
...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:E0A95F,100:C77BA8&height=100&section=
